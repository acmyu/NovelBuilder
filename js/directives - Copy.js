  ...
  return {
      ...
      link: function (scope, element, attrs) {
          ...
          if (angular.isArray(scope.node.children)) {
			  var content = "<ion-item class='no-border no-right-padding'>" +
								"<ion-reorder-button class='ion-navicon' on-reorder='movePane(node, $fromIndex, $toIndex)'></ion-reorder-button>" +
								"<v-pane><v-pane-header>" +
									"<plot-node-content></plot-node-content>" +
								"</v-pane-header>" +
								"<v-pane-content>" +
									"<v-accordion> <pane-buttons></pane-buttons>" +
									(scope.node.children.length==0 ? "" : "<ion-list show-reorder='node.showReorder'><plot-children children='node.children'></plot-children></ion-list>") +
									"</v-accordion>" +
								"</v-pane-content>"+
							"</v-pane></ion-item>";
